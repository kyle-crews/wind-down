class LogsController < ApplicationController
  ...
  post '/logs' do
    if params[:description].empty? || params[:amount].empty? || params[:date].empty? || params[:day_name].empty?
      ...
      @day.user_id = @user.id
      @log = Log.create(description:params[:description], amount:params[:amount], date:params[:date], day_id:@day.id, user_id:@user.id)
  ...
  patch '/logs/:id' do
    if !params[:description].empty? && !params[:amount].empty? && !params[:date].empty?
      @log = Log.find(params[:id])
      @log.update(description:params[:description], amount:params[:amount], date:params[:date])
